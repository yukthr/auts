...
def response_data(Public_IP,Private_IP):
    response = table.query(KeyConditionExpression=Key('Public_IP').eq(Public_IP))
    if response['Count'] == 1:
        return 'IP Already Exists! \n\n{}'.format(response)
    else:
        generate_config(build_yaml(Public_IP,Private_IP),Public_IP,Private_IP)
        return 'Done with all the updates, check devices'

def build_yaml(Public_IP,Private_IP):
    # Build pulic ip , private ip and host to which config needs to be deployed to
    yaml_for_this_build = dict(pu_ip = Public_IP, pi_ip = Private_IP)
...
def generate_config(build_yaml,Public_IP,Private_IP):
    ...
    if response1['ResponseMetadata']['HTTPStatusCode'] == 200 and response2['ResponseMetadata']['HTTPStatusCode'] == 200:
        ...
        invoke_ansible(Public_IP,Private_IP,y=file_final)
    else:
        print('S3 verification failed. Upload probably failed. Exception occured')
        sys.exit(1)
    return file_final

def invoke_ansible(Public_IP,Private_IP,y=None):
    ...
    with open('output','r') as file:
        ...
        if json_output['failed'] == False and json_output['changed']== True:
            ...
            final_update_dynamo(Public_IP)
            cleanup(build_yaml(Public_IP,Private_IP))
            os.system('rm -rf {}'.format(generate_ansible_yml))
            os.system('rm -rf {}'.format(y))
            return "All updates ended"
        else:
...
def final_update_dynamo(Public_IP):
...
def cleanup(y):
    return 'cleaning up file: output'
    return 'cleaning up file: generate_ansible_yml'
